...
class NN_tf:
	...
	def xavier_init(self, size, name):
		...
		return tf.Variable(tf.random.normal(size, mean = 0, stddev = xavier_stddev), dtype=tf.float32, name = name)
	...
	@tf.function
	def construct_Jacobian_dirichlet(self, x_tf, y_tf, t_tf, xi_tf, target, N):
		with tf.GradientTape(persistent = True) as tape:
	...
	def construct_Jacobian_solution(self, x_tf, y_tf, t_tf, xi_tf, target, N):
		with tf.GradientTape(persistent = True) as tape:
			err = self.compute_solution(x_tf, y_tf, t_tf, xi_tf, target)*tf.math.sqrt(self.type_weighting[3]/(N))
			err = tf.reshape(err, [tf.reduce_prod(tf.shape(err)),1])
		weights_jacobians = tape.jacobian(err, self.weights)
		biases_jacobians = tape.jacobian(err, self.biases)
