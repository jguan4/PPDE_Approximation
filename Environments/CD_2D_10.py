...
class CD_2D_10:
	...
	def generate_A_F(self,p):
		...
		v1[0:self.nx] = 1
		v2[range(0,tot_dim,self.nx),0] = (1 - ((1+self.yt)/2))**2
	...
	def create_inner_X(self, p):
		...
		X_f = np.concatenate(((1-np.absolute(X))/P,Y,P),axis=1)
		return X_f
	...
	def f_res(self, x_tf, y_tf, t_tf, xi_tf, u, u_x, u_y, u_t, u_xx, u_yy):
		f_u = -u_xx/xi_tf-xi_tf*u_yy+(1+(tf.math.sign((1-xi_tf*x_tf))*(1-xi_tf*x_tf))**2/4)*u_y
