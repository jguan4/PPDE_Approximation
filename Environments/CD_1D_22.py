...
class CD_1D_22:
	def __init__(self, N_p_train, N_p_test, h, type_weighting = [1,1,1,1], inner = False, sampling_method = 0, add_sample = False, path_env = "./Environments/", L = 0):
		...
		self.x_p_domain = np.array([[-1, 1], [-4, 0]])
		self.h = h
	...
	def compile_output(self, u, p, test):
		if self.sampling_method == 0 or self.sampling_method == 3:
			...
			if not test:
				...
				X1 = self.create_tol_X1(p)
				u_tol = self.u_exact(X_0)
				self.N0_tests = np.concatenate((self.N0_tests, X1),axis = 0) if self.N0_tests.size else X1
	...
	def create_tol_X1(self,p):
		X = self.x.reshape((self.N,1))
		P = p*np.ones((self.N,self.P_dim))
		X1 = (X+np.sign(X))/P-np.sign(X)*1e4
		
		X_f = np.concatenate((X1,P),axis=1)
		return X_f
	...
	def generate_PINN_samples(self, app_str = ""):
		...
		filename = "CD_1D_21_{0}{1}.npz".format(Ns,app_str)
		if os.path.exists("{1}{0}".format(filename,self.path_env)):
			...
			if app_str == "_uniform":
				...
				self.Xf[:,0] = (self.Xf[:,0]+np.sign(self.Xf[:,0]))/self.Xf[:,1]-np.sign(self.Xf[:,0])*1e4
			target_f = np.zeros([self.Nf,1])
			...
			lb = np.concatenate((-1e4*np.ones((self.Nb//3,1)),pb_10),axis = 1)
			ulb = np.zeros((self.Nb//3,1))
			rb = np.concatenate((1e4*np.ones((self.Nb//3,1)),pb_10),axis = 1)
			urb = np.zeros((self.Nb//3,1))
			mb = np.concatenate((1/pb_10-1e4,pb_10),axis = 1)
			umb = 1-np.exp(-1/pb_10)

			self.Xb_d = np.concatenate((lb,rb,mb),axis = 0)
			self.ub_d = np.concatenate((ulb,urb,umb),axis = 0)
			if self.N0>0:
		...
		# print(self.Xb_d)
		# fig, ax = plt.subplots()
		# ax.plot(self.Xf[:,0], self.Xf[:,1], 'o',color ="red")
		# ax.plot(self.Xb_d[:,0], self.Xb_d[:,1], 'o',color ="blue")
		# ax.set_xlabel(r'$x$')
		# ax.set_ylabel(r'$u$')
		# plt.show()
		# input()
	...
	def plot_NN(self, net, figure_save_path = None):
		...
		for i in range(0,self.N_p_test):
			...
			ax.set_xlabel(r'$x$')
			ax.set_ylabel(r'$u$')
			fig.suptitle(r"$\epsilon$ = {0}".format(xi))
			# if figure_save_path is not None:
