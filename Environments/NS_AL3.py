...
class NS_AL3:
	...
	def generate_PINN_samples(self, app_str = ""):
		...
		filename = "NS_AL3_{0}{1}_mat.npz".format(self.Ns,app_str)
		...
		if self.N0>0:
			...
			w1_tf = tf.constant(self.X0[:,[3]],dtype = tf.float32)
			w2_tf = tf.constant(self.X0[:,[4]],dtype = tf.float32)
			t1_tf = tf.constant(self.X0[:,[5]],dtype = tf.float32)
			t2_tf = tf.constant(self.X0[:,[6]],dtype = tf.float32)
	...
	def test_NN(self, net, record_path = None,save_name = None):
		...
		t_tf = tf.constant((),shape = (N,0),dtype = tf.float32)
		x_tf = tf.constant(test_input[:,[0]],dtype = tf.float32)
		y_tf = tf.constant(test_input[:,[1]],dtype = tf.float32)
		xi_tf = tf.constant(test_input[:,[2]],dtype = tf.float32)
		w1_tf = tf.constant(test_input[:,[3]],dtype = tf.float32)
		w2_tf = tf.constant(test_input[:,[4]],dtype = tf.float32)
		t1_tf = tf.constant(test_input[:,[5]],dtype = tf.float32)
		t2_tf = tf.constant(test_input[:,[6]],dtype = tf.float32)
		# target_tf = tf.constant(self.u0_tests, dtype = tf.float32)
		u1_test = tf.constant(test_target[:,[5]],dtype = tf.float32)
		u2_test = tf.constant(test_target[:,[6]],dtype = tf.float32)
		# x_tf = X0_dict["x_tf"]
		# y_tf = X0_dict["y_tf"]
		# t_tf = X0_dict["t_tf"]
		# xi_tf = X0_dict["xi_tf"]
