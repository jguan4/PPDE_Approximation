	...
	for i in range(len(samples_list)):
		...
		if N_int<batch_lim:
			...
			if Ntr_name_i == "Res":
				...
				jacobs_tol_W1t, jacobs_tol_b1t, jacobs_tol_W2t, jacobs_tol_b2t, err_batcht = net.construct_Jacobian_solution_tol(x_i, y_i, t_i, xi_i, output_i, N, weight)
				JJ_Wt1, JJ_bt1, Je_Wt1, Je_bt1 = compute_JJ(jacobs_tol_W1t, jacobs_tol_b1t, err_batcht)
				JJ_W1 = JJ_W1 + JJ_Wt1
				JJ_b1 = JJ_b1 + JJ_bt1
				Je_W1 = Je_W1 + Je_Wt1
				Je_b1 = Je_b1 + Je_bt1

				JJ_Wt2, JJ_bt2, Je_Wt2, Je_bt2 = compute_JJ(jacobs_tol_W2t, jacobs_tol_b2t, err_batcht)
				JJ_W2 = JJ_W2 + JJ_Wt2
				JJ_b2 = JJ_b2 + JJ_bt2
				Je_W2 = Je_W2 + Je_Wt2
				Je_b2 = Je_b2 + Je_bt2
			...
			for j in range(tf.dtypes.cast(N_int/batch_lim, tf.int32)):
				...
				if Ntr_name_i == "Res":
					...
					jacobs_tol_W1t, jacobs_tol_b1t, jacobs_tol_W2t, jacobs_tol_b2t, err_batcht = net.construct_Jacobian_solution_tol(x_batch, y_batch, t_batch, xi_batch, target, N, weight)
					JJ_Wt1, JJ_bt1, Je_Wt1, Je_bt1 = compute_JJ(jacobs_tol_W1t, jacobs_tol_b1t, err_batcht)
					JJ_W1 = JJ_W1 + JJ_Wt1
					JJ_b1 = JJ_b1 + JJ_bt1
					Je_W1 = Je_W1 + Je_Wt1
...
def train_lm_update_tol1(net, samples_list, max_iter, tol, mu, beta, save_toggle, save_for_plot, path_weight = "./temp.npz", path_log = "./Log/", path_plot = "./temp.npz"):
	...
	loss_val_tf = net.loss_tol(samples_list)
	...
	while loss_val>tol and epoch<max_iter and gradient>1e-5 and loss_diff != 0:
		...
		temp_loss, gradient, mu, net = update_weights_tol(net,samples_list,mu, batch_lim)
...
def update_weights_tol(net, samples_list ,mu, batch_lim):
	tau = 1
	...
	loss_val = net.loss_tol(samples_list, save_toggle=False)
	...
	loss_val_tf, grads_W_list1, grads_b_list1, grads_W_list2, grads_b_list2 = net.construct_Gradient_tol1(samples_list)
	...
	loss_val, mu, net = test_mu_tol(net, samples_list, old_weights1, old_biases1, JJ_W_list1, JJ_b_list1, grads_W_list1, grads_b_list1, old_weights2, old_biases2, JJ_W_list2, JJ_b_list2, grads_W_list2, grads_b_list2, loss_val, mu, tau)

	return loss_val, gradient, mu, net


def test_mu_tol(net, samples_list, old_weights1, old_biases1, JJ_W_list1, JJ_b_list1, grads_W_list1, grads_b_list1, old_weights2, old_biases2, JJ_W_list2, JJ_b_list2, grads_W_list2, grads_b_list2, loss_val, mu, tau):
	m = 1
	...
	temp_loss_tf = net.loss_tol(samples_list)
	...
	if loss_diff>=0:
		...
		while loss_diff1>0:
			...
			net.update_weights_biases_uhat(tf.squeeze(-tau*W_update_tol1), tf.squeeze(-tau*b_update_tol1))
			net.update_weights_biases_b(tf.squeeze(-tau*W_update_tol2), tf.squeeze(-tau*b_update_tol2))
			temp_loss_tf1 = net.loss_tol(samples_list)
		...
		net.set_weights_biases_uhat(best_weights1, best_biases1)
		net.set_weights_biases_b(best_weights2, best_biases2)
		loss_current = net.loss_tol(samples_list)
