...
def train_sgd_2du1(net, samples_list, max_iter, tol, save_toggle, save_for_plot, path_weight = "./temp.npz", path_log = "./Log/", path_plot = "./temp.npz"):
	...
	tau, new_weights, new_biases = net.line_search(samples_list, old_weights_lin, old_biases_lin, -tf.squeeze(grads_tol_W), -tf.squeeze(grads_tol_b) ,1)
	...
	while loss_val>tol and gradient>1e-5 :# and loss_diff != 0  and epoch<max_iter: 
		...
		batchsize = 64
		iters = int(np.ceil(400/batchsize))
		for it in range(iters):
			...
			temp_loss, grads_tol_W, grads_tol_b = net.construct_Gradient(sample_batch_list)
