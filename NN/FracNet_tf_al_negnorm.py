...
class FracNet_tf_al_negnorm:
	def __init__(self, input_size, output_size, layers, env, regular_alphas):
		...
		self.loss_b_n_list = []
		self.loss_init_list = []
		self.tau = tf.constant(0.2)
		self.gamma = tf.constant(0.5)

	def load_mat(self):
		self.L = np.load('/home/jj/Documents/Git/PPDE_Approximation/Environments/NS_AL4/NS_AL4_Laplacian_A.npy')
	...
	def forward(self, x_tf, y_tf, t_tf, xi_tf, w1_tf, w2_tf, t1_tf, t2_tf):
		...
		H = X
		outs.append(tf.identity(H))
		W = tf.reshape(self.weights[0], self.weights_dims[0])
		b = self.biases[0]
		H = self.tau**self.gamma*tf.math.exp(tf.math.lgamma(2-self.gamma))*tf.keras.activations.tanh(tf.matmul(H, W) + b)
		outs.append(tf.identity(H))
		# H = (X - self.lb)/(self.ub - self.lb)
		# for l in range(num_layers-1):
		for l in range(1,num_layers-1):
			for k in range(2,l):
				alpha = (l+1-k)**(1-self.gamma)-(l-k)**(1-self.gamma)
				z = z + alpha*(outs[k]-outs[k-1])
			W = tf.reshape(self.weights[l], self.weights_dims[l])
			b = self.biases[l]
			# H = tf.keras.activations.tanh(tf.matmul(H, W) + b)	
			H = H-z+self.tau**self.gamma*tf.math.exp(tf.math.lgamma(2-self.gamma))*tf.keras.activations.tanh(tf.matmul(H, W) + b)	
			outs.append(tf.identity(H))
	...
	def compute_solution_res(self, x_tf, y_tf, t_tf, xi_tf, w1_tf, w2_tf, t1_tf, t2_tf, target):
		u = self.forward(x_tf, y_tf, t_tf, xi_tf, w1_tf, w2_tf, t1_tf, t2_tf)
		rhs = tf.concat((t1_tf,t2_tf),axis = 1)
		# err = u-target
		e = tf.einsum('nij,nj->ni', self.A, u)
		err = e-rhs
		# err = []
	...
	def loss(self, samples_list, save_toggle = False):
		...
		for i in range(len(samples_list)):
			...
			if name_i == "Res":
				...
				loss_0 = 0.0
				err_0 = self.compute_solution_res(x_tf, y_tf, t_tf, xi_tf, w1_tf, w2_tf, t1_tf, t2_tf, target)
				for i in range(err_0.shape[0]):
					err_0i = tf.reshape(err_0[i],[err_0.shape[1],1])
					# print(err_0i)
					# input()
					#<L^{-1}e,e>
					loss_0i = tf.math.reduce_sum(tf.linalg.solve(self.L, err_0i)*err_0i)
	...
	def line_search(self, samples_list, x, y, delta_x, delta_y ,tau, alpha = 0.01):
		...
		x_new = x + incre_x
		y_new = y + incre_y
		rhs = self.set_weights_loss(samples_list, x, y) - alpha*np.sum([np.sum(incre_x*delta_x),np.sum(incre_y*delta_y)])
		while self.set_weights_loss(samples_list, x_new, y_new)>rhs:
			tau = 0.5*tau
			incre_x = tau*delta_x
			incre_y = tau*delta_y
			x_new = x + incre_x
