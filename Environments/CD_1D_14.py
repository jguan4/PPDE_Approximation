...
class CD_1D_14:
	...
	def u_exact(self, X):
		x = X[:,[0]]
		xi = X[:,[1]]
		u = 0.0-xi*x-(np.exp(-x-1.0/xi)-np.exp(-1/xi))/(1-np.exp(-1/xi));
		# u = 1-np.exp((x-1)/xi)
		return u
	...
	def generate_PINN_samples(self, app_str = ""):
		...
		filename = "CD_1D_14ver_{0}{1}.npz".format(Ns,app_str)
		if os.path.exists("{1}{0}".format(filename,self.path_env)):
			...
			lb = np.concatenate((-1.0/pb_10,pb_10),axis = 1)
			ulb = np.zeros((self.Nb//2,1))
			rb = np.concatenate((0.0/pb_10,pb_10),axis = 1)
