...
class CD_1D_26:
	...
	def u_exact(self, X):
		x = X[:,[0]]
		xi = X[:,[1]]
		u = 1-x/xi-(np.exp(-x)-np.exp(-xi))/(1-np.exp(-xi));
		# u = 1-np.exp((x-1)/xi)
		return u
	...
	def generate_PINN_samples(self, app_str = ""):
		...
		filename = "CD_1D_26ver_{0}.npz".format(Ns,app_str)
		if os.path.exists("{1}{0}".format(filename,self.path_env)):
			...
			rb = np.concatenate((1/pb_10,1/pb_10),axis = 1)
