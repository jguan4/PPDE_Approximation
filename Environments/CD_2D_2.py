...
class CD_2D_2:
	...
	def generate_PINN_samples(self, app_str = ""):
		...
		filename = "CD_2D_2ver_{0}{1}.npz".format(Ns,app_str)
		if os.path.exists("{1}{0}".format(filename,self.path_env)):
			...
			if app_str == "_uniform" or app_str == "_uniform_100":
				xnum = 100
				...
				xnum_int = int(np.sqrt(xnum))
				rend = 2/eps
				ymat = np.linspace(np.zeros(rend.shape),rend,xnum_int+2)
				ymat = ymat[1:-1]

				xmat = np.linspace(-np.ones(rend.shape),np.ones(rend.shape),xnum_int+2)
				xmat = xmat[1:-1]

				Xi_arr = np.zeros((self.Nf,1))
				Eta_arr = np.zeros((self.Nf,1))
				for epsi in range(epsnum):
					Xi,Eta = np.meshgrid(xmat[:,0],ymat[:,0])
					Xi_arr[(epsi)*xnum_int**2:(epsi+1)*xnum_int**2] = np.reshape(Xi,(xnum_int**2,1))
					Eta_arr[(epsi)*xnum_int**2:(epsi+1)*xnum_int**2] = np.reshape(Eta,(xnum_int**2,1))

				self.Xf = np.concatenate((Xi_arr,Eta_arr,eps_arr),axis=1)
			elif app_str == "_uniform_400":
				xnum = 400.0
				epsnum = int(self.Nf//xnum)
				eps_log = np.linspace(-4,0,epsnum)
				eps = np.power(10,eps_log)
				eps_arr = np.tile(eps,int(xnum))
				eps_arr = eps_arr.reshape((self.Nf,1))
			...
			xyb_t = (1-xyb)/pb_10
			# z = -1
			lb = np.concatenate((-np.ones((Nb_side,1)),xyb_t,pb_10),axis = 1)
			ulb = -(1-np.exp(-xyb_t))/(1-np.exp(-2/pb_10))
			# z = 1
			rb = np.concatenate((np.ones([Nb_side,1]),xyb_t,pb_10),axis = 1)
			urb = (1-np.exp(-xyb_t))/(1-np.exp(-2/pb_10))
			# t = 0
