...
class NS_AL1:
	def __init__(self, N_p_train, N_p_test, h, type_weighting = [1,1,1,1], inner = False, sampling_method = 0, add_sample = False, path_env = "./Environments/", L = 0):
		...
		# return point-wise solutions, including parameters in input
			...
			self.Ns = N_p_train
			self.lb = np.array([-1.0, -1.0, 1e-4, -2.0, 0.0])
			self.ub = np.array([1.0, 1.0, 1.0, -2e-4, 1.0])
			self.Nf = N_p_train[0]
			self.Nb = N_p_train[1]
			self.Nn = N_p_train[2]
			self.N0 = N_p_train[3]
	...
	def generate_PINN_samples(self, app_str = ""):
		...
		filename = "NS_AL1_{0}{1}.npz".format(self.Ns,app_str)
		if os.path.exists("{1}{0}".format(filename,self.path_env)):
			...
			if app_str == "_random":
				w1 = np.random.rand(self.Nf,1)
				w2 = np.random.rand(self.Nf,1)
			else:
				w1 = -2*self.Xf[:,[2]]
				w2 = 0*self.Xf[:,[2]]
			...
			lb = np.concatenate((-np.ones((Nb_side,1)),xyb,pb_10,-2*pb_10,np.zeros((Nb_side,1))),axis = 1)
			...
			tb = np.concatenate((xyb,np.ones((Nb_side,1)),pb_10,-2*pb_10,np.zeros((Nb_side,1))),axis = 1)
			utb = np.zeros((Nb_side,2))
			db = np.concatenate((xyb,-np.ones((Nb_side,1)),pb_10,-2*pb_10,np.zeros((Nb_side,1))),axis = 1)
	...
	def f_res(self, x_tf, y_tf, t_tf, xi_tf, w1_tf, w2_tf, u1, u2, u1_x, u1_y, u1_t, u1_xx, u1_yy, u1_xy, u2_x, u2_y, u2_t, u2_xx, u2_yy, u2_xy):
		f_u1 = -xi_tf*(u1_xx+u1_yy)+u1*u1_x+u2*u1_y+u1_xx+u2_xy+w1_tf
		f_u2 = -xi_tf*(u2_xx+u2_yy)+u1*u2_x+u2*u2_y+u2_yy+u1_xy+w2_tf
		return f_u1, f_u2
