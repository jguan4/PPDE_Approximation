...
class CD_1D_17:
	...
	def u_exact(self, X):
		x = X[:,[0]]
		xi = X[:,[1]]
		u = 1-np.sqrt(xi)*x-xi*(np.exp(-x/np.sqrt(xi))-np.exp(-1/xi))
		# u = 1-np.exp((x-1)/xi)
		return u
	...
	def generate_PINN_samples(self, app_str = ""):
		...
		filename = "CD_1D_17ver_{0}{1}.npz".format(Ns,app_str)
		if os.path.exists("{1}{0}".format(filename,self.path_env)):
			...
			if app_str == "_uniform":
				...
				self.Xf[:,0] = (1-self.Xf[:,0])/np.sqrt(self.Xf[:,1])
			...
			lb = np.concatenate((1/np.sqrt(pb_10),pb_10),axis = 1)
	...
	def f_res(self, x_tf, y_tf, t_tf, xi_tf, u, u_x, u_y, u_t, u_xx, u_yy):
		f_u = -u_xx-u_x/tf.math.sqrt(xi_tf)-1
