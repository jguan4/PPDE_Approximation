...
class CD_1D_20:
	...
	def generate_PINN_samples(self, app_str = ""):
		...
		filename = "CD_1D_20_{0}{1}.npz".format(Ns,app_str)
		if os.path.exists("{1}{0}".format(filename,self.path_env)):
			...
			lb = np.concatenate((-np.ones((self.Nb//3,1)),pb_10),axis = 1)
			ulb = np.zeros((self.Nb//3,1))
			rb = np.concatenate((np.ones([self.Nb//3,1]),pb_10),axis = 1)
			urb = np.zeros((self.Nb//3,1))
			mb = np.concatenate((np.zeros([self.Nb//3,1]),pb_10),axis = 1)
	...
	def f_res(self, x_tf, y_tf, t_tf, xi_tf, u, u_x, u_y, u_t, u_xx, u_yy):
		f_u = -u_xx*xi_tf+tf.math.sign(x_tf)*u_x
		return f_u
