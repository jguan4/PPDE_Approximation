...
class CD_1D_4:
	...
	def u_exact(self, X):
		x = X[:,[0]]
		xi = X[:,[1]]
		u = xi*x-(np.exp(x-1/xi)-np.exp(-1/xi))/(1-np.exp(-1/xi));
	...
	def generate_PINN_samples(self, app_str = ""):
		...
		filename = "CD_1D_4ver_{0}{1}.npz".format(Ns,app_str)
	...
	def plot_NN(self, net, figure_save_path = None):
		...
		for i in range(0,self.N_p_test):
			...
			ax.plot(self.x, fres_i,'--',color="green")
