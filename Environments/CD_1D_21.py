...
class CD_1D_21:
	...
	def create_tol_X(self,p):
		...
		X_f = np.concatenate(((1-np.absolute(X))/P,P),axis=1)
		return X_f
	...
	def generate_PINN_samples(self, app_str = ""):
		...
		if os.path.exists("{1}{0}".format(filename,self.path_env)):
			...
			if app_str == "_uniform":
				...
				self.Xf[:,0] = (1-np.absolute(self.Xf[:,0]))/self.Xf[:,1]
			target_f = np.zeros([self.Nf,1])
			...
			lb = np.concatenate((-np.zeros((self.Nb//3,1)),pb_10),axis = 1)
			ulb = np.zeros((self.Nb//3,1))
			rb = np.concatenate((np.zeros([self.Nb//3,1]),pb_10),axis = 1)
			urb = np.zeros((self.Nb//3,1))
			mb = np.concatenate((1/pb_10,pb_10),axis = 1)
