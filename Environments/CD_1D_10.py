...
class CD_1D_10:
	def __init__(self, N_p_train, N_p_test, h, type_weighting = [1,1,1,1], inner = False, sampling_method = 0, add_sample = False, path_env = "./Environments/", L = 0):
		...
		# return point-wise solutions, including parameters in input
			...
			self.lb = np.array([-7.5e3, 1e-4])
			self.ub = np.array([2.5e3, 1.0])
			self.Nf = N_p_train[0]
			self.Nb = N_p_train[1]
			self.Nn = N_p_train[2]
			self.N0 = N_p_train[3]
	...
	def u_exact(self, X):
		x = X[:,[0]]
		xi = X[:,[1]]
		u = self.a-xi*x-(np.exp(-x-(1-self.a)/xi)-np.exp(-1/xi))/(1-np.exp(-1/xi));
		# u = 1-np.exp((x-1)/xi)
		return u
	...
	def generate_PINN_samples(self, app_str = ""):
		...
		filename = "CD_1D_9ver_{0}{1}.npz".format(Ns,app_str)
		if os.path.exists("{1}{0}".format(filename,self.path_env)):
			...
			self.Xf[:,0] = (self.a-self.Xf[:,0])/self.Xf[:,1]
			...
			lb = np.concatenate((self.a/pb_10,pb_10),axis = 1)
			ulb = np.zeros((self.Nb//2,1))
			rb = np.concatenate(((self.a-1)/pb_10,pb_10),axis = 1)
