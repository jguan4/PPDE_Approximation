...
class CD_1D_27:
	def __init__(self, N_p_train, N_p_test, h, type_weighting = [1,1,1,1], inner = False, sampling_method = 0, add_sample = False, path_env = "./Environments/", L = 0):
		...
		# return point-wise solutions, including parameters in input
			...
			self.lb = np.array([1e-4, 1.0])
			self.ub = np.array([1.0, 1e4])
			self.Nf = N_p_train[0]
			self.Nb = N_p_train[1]
			self.Nn = N_p_train[2]
			self.N0 = N_p_train[3]
	...
	def u_exact(self, X):
		x = X[:,[0]]
		xi = X[:,[1]]
		u = 1-x*xi-(np.exp(-x*(xi**2))-np.exp(-xi))/(1-np.exp(-xi));
		# u = 1-np.exp((x-1)/xi)
		return u
	...
	def create_tol_X(self,p):
		...
		X_f = np.concatenate((X,1/P),axis=1)
		return X_f
	...
	def generate_PINN_samples(self, app_str = ""):
		...
		filename = "CD_1D_27ver_{0}.npz".format(Ns,app_str)
		if os.path.exists("{1}{0}".format(filename,self.path_env)):
			...
			if app_str == "_uniform":
				...
				self.Xf = np.concatenate((x_arr,1/eps_arr),axis=1)
			else:
			...
			lb = np.concatenate((np.zeros((self.Nb//2,1)),1/pb_10),axis = 1)
			ulb = np.zeros((self.Nb//2,1))
			rb = np.concatenate((pb_10,1/pb_10),axis = 1)
